...
def generate_testcases(req_data, debug):
    print("Let's generate Test Cases: ")
    if debug: print(req_data)
    ai_data = generate_with_openai(req_data, debug)
    if debug: print(ai_data)
 
def generate_with_openai(prompt, debug):
...
def main(req, username, baseurl, debug):
    ...
    if input("Generate Test Cases: y/n?") == "y":
        generate_testcases(req_data, debug)
...
if __name__ == "__main__":
    ...
    main(args.req, args.username, args.url, args.debug)
